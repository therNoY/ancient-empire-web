  ...
  methods: {
    ...
    moveUnit() {
      ...
      if (this.movePointIndex + 1 == this.mapDt.pathPoints.length) {
        let time = this.mapDt.pathPoints[this.mapDt.pathPoints.length - 2].length * 250;
    ...
    getMoveLength() {
      ...
      if (point1.row == point2.row) {
        this.$store.commit("changeMoveLength", Math.abs(point1.column - point2.column));
      } else {
        this.$store.commit("changeMoveLength", Math.abs(point1.row - point2.row));
      }
    },
